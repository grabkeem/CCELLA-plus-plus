...
class ReconModelRaw(torch.nn.Module): # Riffed off sample.py
    """
    A PyTorch module for reconstructing images from latent representations.
    ...
    def forward(self, z, scale_factor):
        ...
        Returns:
            torch.Tensor: The reconstructed image.
        """
        recon_pt_nda = self.autoencoder.decode_stage_2_outputs(z / scale_factor)
        return recon_pt_nda
...
def add_data_dir2path_keys_base(list_files: list, data_dir: str, embedding_dir: str, imkey: str, lblkey: str) -> list:
    """
    Read a list of data dictionary.
    2024-DEC-06 created (EPG)

    Args:
        list_files (list): input data to load and transform to generate dataset for model.
        data_dir (str): directory of files.
        imkey (str): Image key from embedding_dir
        lblkey (str): Label key from data_dir
    ...
    new_list_files = copy.deepcopy(list_files)
    for d in new_list_files:
        d[imkey] = os.path.join(embedding_dir, d[imkey].removesuffix('.nii.gz')+'_emb.nii.gz')

        d[lblkey] = os.path.join(data_dir, d[lblkey])

        d["top_region_index"] = np.array([0,0,0,1],dtype=np.float32)
        d["bottom_region_index"] = np.array([0,0,0,1],dtype=np.float32)
        d["spacing"] = np.array([0.5,0.5,0.75],dtype=np.float32)
        

    return new_list_files


def add_data_dir2path_keys(list_files: list, data_dir: str, embedding_dir: str, imkey: str, lblkey: str) -> list:
    """
    Read a list of data dictionary.
    2024-DEC-06 created (EPG)
    ...
    Args:
        list_files (list): input data to load and transform to generate dataset for model.
        data_dir (str): directory of files.
        imkey (str): Image key from embedding_dir, add _emb to each
        lblkey (str): Label key from data_dir
...
def add_data_dir2path_keys_base_eval(list_files: list, data_dir: str, embedding_dir: str, imkey: str, lblkey: str) -> list:
    """
    Read a list of data dictionary.
    2024-DEC-10 created (EPG)
    ...
    for d in new_list_files:
        if imkey == 'multi':
            d["axt2_orig"] = os.path.join(data_dir, d['axt2'])
            d["b1600_orig"] = os.path.join(data_dir, d['b1600'])
            d["adc_orig"] = os.path.join(data_dir, d['adc'])
            d['multi'] = os.path.join(embedding_dir, d['axt2'].replace("axt2.nii.gz", "multi_emb.nii.gz"))
        else:
            d[f"{imkey}_orig"] = os.path.join(data_dir, d[imkey])
            d[imkey] = os.path.join(embedding_dir, d[imkey].removesuffix('.nii.gz')+'_emb.nii.gz')
...
def encode_text(text, tokenizer, text_encoder, device, max_length=256):
...
def add_data_dir2path_keys_eval_text(list_files: list, data_dir: str, embedding_dir: str, imkey: str, lblkey: str) -> list:
    """
    Read a list of data dictionary.
    2024-DEC-10 created (EPG)
...
def add_data_dir2path_keys_eval(list_files: list, data_dir: str, embedding_dir: str, imkey: str, lblkey: str) -> list:
    """
    Read a list of data dictionary.
    2024-DEC-10 created (EPG)
...
def load_filenames(data_list_path: str) -> list:
    """
    Load filenames from the JSON data list. Note that JSON is encrypted and needs decryption first
    ...
    """
    if data_list_path.endswith('.enc'):
        with open("./scripts/masterkey.key", "rb") as key_file:
            key = key_file.read()
        cipher_suite = Fernet(key)
        with open(data_list_path, "rb") as f:
            encrypted_data = f.read()
        json_string = cipher_suite.decrypt(encrypted_data).decode()

        filenames_train = json.loads(json_string)
    elif data_list_path.endswith('.json'):
        with open(data_list_path, "r") as f:
            filenames_train = json.load(f)
    else:
        raise ValueError("Data list file must be either .json or .enc")
    return filenames_train

def prepare_maisi_controlnet_json_dataloader_path(
    ...
    if isinstance(json_data_list, list):
        ...
        for data_list, data_root, embedding_root in zip(json_data_list, data_base_dir, embedding_base_dir):
            ...
            train = add_data_dir2path_keys(json_data, data_root, embedding_root, imkey, lblkey)
            list_train += train
    else:
        json_data = load_filenames(json_data_list)
        list_train = add_data_dir2path_keys(json_data, data_base_dir, embedding_base_dir, imkey, lblkey)

    common_transform = [
        LoadImaged(keys=[imkey, lblkey], image_only=True, ensure_channel_first=True),
        Orientationd(keys=[lblkey], axcodes="RAS"),
        EnsureTyped(keys=[lblkey], dtype=torch.uint8, track_meta=True),
...
def prepare_maisi_controlnet_json_dataloader_base(
    json_data_list: list | str,
    data_base_dir: list | str,
    embedding_base_dir: list | str,
    imkey: str,
    ...
    """
    Prepare dataloader for base MAISI DM
    ...
    if isinstance(json_data_list, list):
        ...
        for data_list, data_root, embedding_root in zip(json_data_list, data_base_dir, embedding_base_dir):
            json_data = load_filenames(data_list)
            train = add_data_dir2path_keys_base(json_data, data_root, embedding_root, imkey, lblkey)
            list_train += train
    else:
        json_data = load_filenames(json_data_list)
        list_train = add_data_dir2path_keys_base(json_data, data_base_dir, embedding_base_dir, imkey, lblkey)
...
def prepare_maisi_controlnet_json_dataloader_path_eval(
    json_data_list: list | str,
    data_base_dir: list | str,
    embedding_base_dir: list | str,
    imkey: str,
    ...
    if isinstance(json_data_list, list):
        ...
        for data_list, data_root, embedding_root in zip(json_data_list, data_base_dir, embedding_base_dir):
            json_data = load_filenames(data_list)
            train = add_data_dir2path_keys_eval(json_data, data_root, embedding_root, imkey, lblkey)
            list_train += train
    else:
        json_data = load_filenames(json_data_list)
        list_train = add_data_dir2path_keys_eval(json_data, data_base_dir, embedding_base_dir, imkey, lblkey)

    if imkey == 'multi':
        orig_keys=["axt2_orig", "b1600_orig", "adc_orig"]
    else:
        orig_keys = [f"{imkey}_orig"]

    common_transform = [
        LoadImaged(keys=[imkey, lblkey], image_only=True, ensure_channel_first=True),
        LoadImaged(keys=orig_keys),
        EnsureChannelFirstd(keys=orig_keys),
        Orientationd(keys=orig_keys+[lblkey], axcodes="RAS"),
        EnsureTyped(keys=orig_keys, dtype=torch.float32),
        EnsureTyped(keys=[lblkey], dtype=torch.uint8, track_meta=True),
        Lambdad(keys="pirads", func=lambda x: torch.FloatTensor(x)),
        Lambdad(keys="spacing", func=lambda x: torch.FloatTensor(x)),
        Lambdad(keys=["pirads", "spacing"], func=lambda x: x * 1e2),
    ...
    if "axt2_orig" in orig_keys:
        common_transform += [ScaleIntensityRangePercentilesd(keys='axt2_orig',lower=0, upper=99.5, b_min=0, b_max=1, clip=True)]
    if "b1600_orig" in orig_keys:
        common_transform += [ScaleIntensityRangePercentilesd(keys='b1600_orig',lower=0, upper=99.5, b_min=0, b_max=1, clip=True)]
    if "adc_orig" in orig_keys:
        common_transform += [ScaleIntensityRanged(keys='adc_orig',a_min=0,a_max=3000.0, b_min=0, b_max=1, clip=True)]
...
class EncodeText(MapTransform):
    ...
    def __call__(self, data):
        for key in self.keys:
            ...
            text_encoding = self.text_encoder(text_inputs)
            data[key] = text_encoding.last_hidden_state
        return data

def prepare_maisi_controlnet_json_dataloader_text_eval(
    json_data_list: list | str,
    data_base_dir: list | str,
    embedding_base_dir: list | str,
    imkey: str,
    ...
    if isinstance(json_data_list, list):
        ...
        for data_list, data_root, embedding_root in zip(json_data_list, data_base_dir, embedding_base_dir):
            json_data = load_filenames(data_list)
            train = add_data_dir2path_keys_eval_text(json_data, data_root, embedding_root, imkey, lblkey)
            list_train += train
    else:
        json_data = load_filenames(json_data_list)
        list_train = add_data_dir2path_keys_eval_text(json_data, data_base_dir, embedding_base_dir, imkey, lblkey)
...
def prepare_maisi_controlnet_json_dataloader_base_eval(
    json_data_list: list | str,
    data_base_dir: list | str,
    embedding_base_dir: list | str,
    imkey: str,
    ...
    if isinstance(json_data_list, list):
        ...
        for data_list, data_root, embedding_root in zip(json_data_list, data_base_dir, embedding_base_dir):
            json_data = load_filenames(data_list)
            train = add_data_dir2path_keys_base_eval(json_data, data_root, embedding_root, imkey, lblkey)
            list_train += train
    else:
        json_data = load_filenames(json_data_list)
        list_train = add_data_dir2path_keys_base_eval(json_data, data_base_dir, embedding_base_dir, imkey, lblkey)
