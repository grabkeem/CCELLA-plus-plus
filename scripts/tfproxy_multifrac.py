...
def prep(args,train_files, val_files, test_files_inst2, test_files_inst1, test_files_all, train_flag=False,val_flag=False):
    ...
    if val_flag: # Testing
        ...
        model.eval()
        # Otherwise models already exist # Nvm let's reload the best instead of the last
        try:
...
def train(args,device,train_loader,val_loader,model_path):
    ...
    val_post_transforms = Compose(
        [EnsureTyped(keys=["pred","label"],device=device),
         ...
         Activationsd(keys="pred", softmax=True),#, AsDiscreted(keys="pred", argmax=True) # threshold=0.5
         CopyItemsd(keys="pred",times=2,names=["pred_binary", "pred_single"]),
         AsDiscreted(keys="pred_binary", argmax=True, keepdim=True), # threshold=0.5
         Lambdad(keys="pred_single", func=lambda x: x[1]), # take the second channel
         AsDiscreted(keys="label", to_onehot=2)
    ...
    evaluator = SupervisedEvaluator(
        ...
        key_val_metric={
            "val_ap": AveragePrecision(output_transform=from_engine(["pred_single", "label_binary"]), device=device), # pred, label without lambda in train
        },
        additional_metrics={
            "val_auc": ROCAUC(average="macro", output_transform=from_engine(["pred", "label"])), # pred, label without lambda in train
            ...
            "val_loss": Loss(loss_fn=loss, output_transform=from_engine(["pred_raw", "label_raw"]), device=device), # pred, label without lambda in train
        },
...
def main():
    ...
    parser.add_argument("--pfrac_continue", type=float, default=0, help="Continue pfrac from scratch at this number")
    parser.add_argument("--pfrac_only", type=float, default=0, help="Only pfrac from scratch for this number")
    ...
    if args.pfrac_continue!=0 and args.pfrac_only!=0:
        raise ValueError(f"Cannot pfrac_continue and pfrac_only")

    if args.pfrac_continue!=0 and args.pfrac_continue not in pfrac_list:
        raise ValueError(f"Got {args.pfrac_continue} for pfrac continue but list is {pfrac_list}")
    
    if args.pfrac_only!=0 and args.pfrac_only not in pfrac_list:
        raise ValueError(f"Got {args.pfrac_only} for pfrac only but list is {pfrac_list}")

    for pfrac in pfrac_list:
        if args.pfrac_continue!=0:
            if args.pfrac_continue==pfrac:
                args.pfrac_continue = 0 # Continue from here
                print(f"Continuing multifrac starting at pfrac {pfrac}")
            else:
                continue # Else skip
        if args.pfrac_only!=0:
            if args.pfrac_only==pfrac:
                print(f"Only running multifrac at pfrac {pfrac}")
            elif pfrac > args.pfrac_only:
                break
            else:
                continue 
        args.pfrac = pfrac/100
        ...
        train_files_inst2_pos = []
        train_files_inst2_neg = []
        ...
        if args.imkey=="multi":
            for idx, row in train_df.iterrows():
                ...
                if not is_inst1: # If inst2 flag set and data is inst2
                    ...
                    if row["ISUP_real"] == 1:
                        train_files_inst2_pos.append({"axt2": axt2_path,
                                                        "highb": highb_path,
                                                        "adc": adc_path,
                                                        "label": row["ISUP_real"]})
                    elif row["ISUP_real"] == 0:
                        train_files_inst2_neg.append({"axt2": axt2_path,
                                                        "highb": highb_path,
                                                        "adc": adc_path,
                                                        "label": row["ISUP_real"]})
                    else:
                        raise ValueError(f"Unknown label {row['ISUP_real']}")
        else:
            for idx, row in train_df.iterrows():
                folder = row["folder"]
                is_inst1 = row["dataset"] == "Institute1"
                if not is_inst1: # If inst2 flag set and data is inst2
                    ...
                    if row["ISUP_real"] == 1:
                        train_files_inst2_pos.append({"image": img_path, "label": row["ISUP_real"]})
                    elif row["ISUP_real"] == 0:
                        train_files_inst2_neg.append({"image": img_path, "label": row["ISUP_real"]})
                    else:
                        raise ValueError(f"Unknown label {row['ISUP_real']}")
        ...
        if args.imkey=="multi":
            for idx, row in val_df.iterrows():
                ...
                if not is_inst1:
                    ...
                    highb_path = os.path.join(real_folder, folder,'b1600.nii.gz')
                    adc_path = os.path.join(real_folder, folder,'adc.nii.gz')
                    val_files_inst2.append({"axt2": axt2_path,
                                            "highb": highb_path,
                                            "adc": adc_path,
                                            "label": row["ISUP_real"]})
        else:
            for idx, row in val_df.iterrows():
                ...
                if not is_inst1:
                    img_path = os.path.join(real_folder, folder,f'{args.imkey}.nii.gz')
                    val_files_inst2.append({"image": img_path, "label": row["ISUP_real"]})
        ...
        # Only give inst2 for val_files, train_files is a fractional selection of Institute2
        val_files = val_files_inst2
        print(f"Total inst2 before: {len(train_files_inst2_pos)+len(train_files_inst2_neg)}")
        num_pos = round(len(train_files_inst2_pos)*args.pfrac)
        num_neg = round(len(train_files_inst2_neg)*args.pfrac)
        train_files = random.sample(train_files_inst2_pos,num_pos) + random.sample(train_files_inst2_neg,num_neg) # Random selection balanced
        print(f"Total inst2 after: {len(train_files)}")

        # for finetune in [False, True]:
        if args.finetune is None or args.finetune == False:
            args.inst2_data = True
            args.inst1_data = None
            args.generated_label = None
            args.add_to_modelname = f"p{pfrac}"
        elif args.finetune:
            args.inst2_data = True
            args.inst1_data = True
            args.generated_label = 1
            args.add_to_modelname = f"p{pfrac}"
        # Validate args:
        ...
        if args.finetune:
            ...
            args.num_epochs = args.num_epochs_finetune # 375
        elif args.inst2_data:
            print(f"This is a train on Institute2, using {args.num_epochs_inst2} for epochs")
            args.num_epochs = args.num_epochs_inst2 # 750
        elif args.inst1_data:
            print(f"This is a train on Institute1, using {args.num_epochs_inst1} for epochs")
            args.num_epochs = args.num_epochs_inst1 # 300
        else:
            raise ValueError("Unknown training paradigm")

        print(f"Starting multifrac: P{pfrac}, train set size {len(train_files)}")
        prep(args,train_files, val_files, test_files_inst2, test_files_inst1, test_files_all, train_flag=not args.validate, val_flag=True)
